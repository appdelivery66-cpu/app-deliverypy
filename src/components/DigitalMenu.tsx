...
export default function DigitalMenu() {
    ...
    // Cargar pedido de localStorage
    useEffect(() => {
        const savedId = localStorage.getItem('last_order_id');
        if (savedId) setLastOrderId(savedId);
    }, []);

    // Polling del status del pedido (cada 20 segundos si hay un pedido activo)
    useEffect(() => {
    ...
    const getLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            () => {
                setLocationStatus("Error al obtener ubicación. Verifique los permisos.");
            }
        );
    };
    ...
    const formatWhatsAppMessage = () => {
        ...
        cart.forEach(item => {
            ...
            message += `• ${item.quantity}x ${item.product.name}${extrasText} - $ ${itemTotal.toFixed(2)}\n`;
        });
        ...
        if (neighborhood) {
            message += `\n*Entrega (${neighborhood.name}): $ ${neighborhood.fee.toFixed(2)}*`;
        }
        ...
        message += `*Forma de Pago:* ${paymentMethod === 'PIX' ? 'Transferencia' : paymentMethod === 'CARD' ? 'Tarjeta' : 'Efectivo'}\n`;

        if (paymentMethod === 'PIX') {
            message += `\n🚨 *Atención:* ¡Por favor, envie el *COMPROBANTE DE PAGO* abajo para que podamos preparar su pedido!`;
        }
    ...
    const checkout = async () => {
        ...
        try {
            ...
            alert(`Aviso: El pedido será enviado a WhatsApp, pero no fue guardado en el Panel Admin.\nMotivo: ${error.message}`);
            const cleanPhone = data.store.whatsapp.replace(/\D/g, '');
            const whatsappUrl = `https://wa.me/${cleanPhone}?text=${formatWhatsAppMessage()}`;
            window.location.href = whatsappUrl;
            setCart([]);
                        ...
                        >
                            <div className="w-32 h-full flex-shrink-0 relative">
                                <img
                                    ...
                                    onError={(e) => {
                                        ...
                                        target.src = "https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=500&auto=format&fit=crop&q=60";
                                    }}
                                />
                            ...
                            <div className="flex-1 p-3 flex flex-col justify-between">
                                ...
                                <div className="flex items-center justify-between">
                                    <span className="text-primary font-black text-base">$ {product.price.toLocaleString('es-ES', { minimumFractionDigits: 2 })}</span>
                                    <button
                ...
                <div className="fixed bottom-4 left-4 right-4 z-50 animate-in slide-in-from-bottom duration-500 max-w-lg mx-auto">
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2">
                            <span className="text-secondary text-lg">$ {totalPrice.toLocaleString('es-ES', { minimumFractionDigits: 2 })}</span>
                ...
                <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-300">
                    ...
                    <div className="relative bg-white w-full max-w-lg rounded-t-[32px] sm:rounded-[32px] overflow-hidden flex flex-col max-h-[90vh] shadow-2xl animate-in slide-in-from-bottom duration-500">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-4">
                            {cart.map((item) => (
                                <div key={item.product.id} className="flex items-center gap-4 py-2 border-b border-muted/50 last:border-0">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-primary font-bold text-sm">$ {getItemPrice(item).toLocaleString('es-ES', { minimumFractionDigits: 2 })}</p>
                                    </div>
                            ...
                            <div className="pt-4 border-t border-dashed border-slate-200">
                                <h4 className="font-bold text-slate-800 mb-3 text-sm flex items-center gap-2">
                                    <X className="w-4 h-4 text-primary rotate-45" /> Cupón de Descuento
                                </h4>
                                ...
                                {appliedCoupon && (
                                    <p className="text-[10px] text-green-600 font-bold mt-1 ml-1 flex items-center gap-1">
                                        ...
                                        Cupón {appliedCoupon.code} aplicado! (- $ {discountAmount.toFixed(2)})
                                    </p>
                                )}
                            </div>
                                    ...
                                    {data.store.acceptsCard && (
                                        <label className={cn(
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <span className="text-xs text-slate-500">Llevamos la terminal</span>
                                            </div>
                                        </label>
                                    )}
                        ...
                        <div className="p-6 bg-muted/10 space-y-4 border-t shadow-[0_-10px_20px_rgba(0,0,0,0.05)]">
                            <div className="space-y-2 py-2 border-b border-white/20">
                                ...
                                {neighborhood && (
                                    <div className="flex items-center justify-between text-sm opacity-80">
                                        ...
                                        <span>$ {neighborhood.fee.toFixed(2)}</span>
                                    </div>
                                )}
                            </div>
                            ...
                            <p className="text-[10px] text-center text-muted-foreground uppercase tracking-widest font-bold">
                                El pedido será enviado directo a nuestro WhatsApp
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="relative bg-white w-full max-w-lg rounded-t-[32px] sm:rounded-[32px] overflow-hidden flex flex-col max-h-[90vh] shadow-2xl animate-in slide-in-from-bottom duration-500">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <h3 className="font-black text-slate-800 uppercase tracking-widest text-sm flex items-center gap-2">
                                            <Plus className="w-4 h-4 text-primary" /> ¿Mejorar tu pedido?
                                        </h3>
                                    ...
                                    <div className="grid grid-cols-1 gap-2">
                                        {selectedProduct.extras.map((extra: any) => (
                                            ...
                                            >
                                                ...
                                                <span className="font-black text-primary text-sm">+ $ {extra.price.toFixed(2)}</span>
                                            </label>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 italic text-slate-500 text-xs">
                                Utilice las observaciones en el carrito para quitar ingredientes (ej: sin cebolla).
                            </div>
                        </div>

                        <div className="p-6 bg-white border-t flex items-center gap-4">
                            <div className="flex-1">
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total del Artículo</p>
                                <p className="text-xl font-black text-primary">
                                    $ {(selectedProduct.price + selectedExtras.reduce((acc, e) => acc + e.price, 0)).toFixed(2)}
                                </p>
                            </div>
                            <button
                                onClick={() => {
